-- HeatDetection.lua
Utils.LogModuleLoaded("HeatDetection")
...
local function FireLog(msg)
    Utils.LogIf("fire", tostring(msg))
end

local function FireTrace(key, intervalSec, msg)
    Utils.ThrottledLog("fire_trace", key, intervalSec, tostring(msg))
end
...
if _empty(FD.classes) and _empty(FD.name) and _empty(FD.prefab) and _empty(FD.particle) then
    FireLog("[HeatDetection]: fireDetection has no match rules")
end
...
function HeatDetection.HasNearbyFireSource(radius)
    ...
    FireTrace("radius", 5,
        ("[HeatDetection->HasNearbyFireSource]: radius=%.2f indoors=%s"):format(radius, tostring(indoors)))
    ...
    FireTrace("scan_count", 5, ("[HeatDetection]: scanning %d entities"):format(#entities))

    local bestStrength = 0.0
    local positiveHits = 0
    local closestPosDist = math.huge
    ...
    for _, e in ipairs(entities) do
        ...
        FireTrace("entity_" .. tostring(why), 10, string.format("[HeatDetection]: scan s=%.2f neg=%s eff=%.2f :: %s",
    ...
    local inProx = (closestPosDist <= prox)
    ...
    if rawGot and rawStrength > 0 then
        ...
        if not S.active and S.pos >= ON then
            S.active = true
            FireLog(string.format("[HeatDetection]: ON pos=%d strength=%.2f", S.pos, S.strength))
        end
    else
        S.neg = (S.neg or 0) + 1
        S.pos = 0
        if S.active and S.neg >= OFF then
            S.active = false
            S.strength = 0.0
            FireLog(string.format("[HeatDetection]: OFF neg=%d", S.neg))
        end
    end
